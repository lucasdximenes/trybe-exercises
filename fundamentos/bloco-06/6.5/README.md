...
2. Make the font size of h1 elements smaller;
...
6. Create a media query in your CSS file, using the pixel dimension you saved as the min-width of the media query test. Inside that breakpoint, add the following tweaks:
...
8. Create a new media query in your CSS file using the dimension you found for large screens (for example 1300px), and make the following adjustments inside the breakpoint:
   ...
   - Adjust the font size;
   - Adjust page margins;
   - Add the max-width property to the page, to ensure that the width of the lines is not too big.
...
2. To do this properly, you must pay attention to the content on the page and think about how to display it on each of the screen sizes:
...
3. Maybe you need to change the HTML a bit, adding elements to make styling easier, or maybe moving a block around within the page.

---

## Part III - Creating mobile and print layouts
...
1. Add a media query to the CSS file and the rules necessary for the page to look like the image below when it is printed. Specifically:
   - Elements with id header, navigation and footer must disappear;
...
2. Add a media query in the CSS file and the necessary rules for the page to look like the images below when the screen is resized to smaller widths. Specifically:
   ...
   - The items inside the navigation element must appear each on its own line;
